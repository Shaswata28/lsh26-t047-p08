...
export default function MarksheetPage() {
    ...
    return (
      <div className="min-h-screen bg-slate-100 flex items-center justify-center p-4">
        <div className="flex items-center space-x-2 text-slate-600 text-sm font-semibold">
          ...
          <span>Generating Official Marksheet...</span>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-slate-100 flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm max-w-md text-center space-y-4">
          ...
          <p className="text-xs text-slate-500">
            Could not find student marksheet for identifier: <strong className="font-mono">{id}</strong>.
          </p>
          <Link
            href="/"
  ...
  return (
    <div className="min-h-screen bg-slate-100 py-8 px-4 sm:px-6 lg:px-8">
      <MarksheetView result={studentResult} />
    </div>
  );
}
